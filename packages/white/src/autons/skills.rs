...
impl Robot {
    pub async fn skills(&mut self){
        ...
        _ = linear.speed(0.75).drive_to_point(dt, Vec2::new(23.5, 23.0), true).await; 
        ...
        _ = turn.turn_to(dt, -Angle::QUARTER_TURN).await;
        _ = linear.drive_distance(dt, -67.0).await;
        _ = turn.turn_to(dt, Angle::ZERO).await;
        ...
        zip(
            async {
                _ = linear.timeout(Duration::from_millis(3000)).speed(0.6).drive_distance(dt, 20.0).await;
            },
            async {
        ...
        zip(
            ...
            async {
                ...
                _ = self.duck_bill.set_high();
                self.lever.score(LeverStage::Score(4.0, false));
            },
        ).await;
