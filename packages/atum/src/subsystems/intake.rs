...
use alloc::rc::Rc;
use vexide::{prelude::OpticalSensor, task::{spawn, Task}, time::sleep};

use crate::{backend::{Settings, SlintColor}, hardware::motor_group::MotorGroup};
...
impl Intake {
    pub fn new(mut motors: MotorGroup, optical_sensor: OpticalSensor, settings: Rc<RefCell<Settings>>) -> Self {
            ...
            _task: spawn(async move {
                loop {
                    ...
                    if *color_sort.borrow() {
                        // Blue balls has a hue of 120 to 240 and red ring has a hue of 0 to 60
