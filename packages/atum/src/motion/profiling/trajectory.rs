...
use crate::{motion::profiling::bezier::Bezier, pose::{Pose, Vec2}};

pub struct TrajectoryConstraints{
    pub max_velocity: f64, // inches/sec
    pub max_acceleration: f64,// inches/sec^2
    pub max_deceleration: f64,// inches/sec^2
    pub friction_coefficient: f64, 
    pub track_width: f64, // inches
}
impl TrajectoryConstraints{
    pub fn max_speed(&self, curvature:f64) -> f64{
        let max_turn_speed = ((2.0 * self.max_velocity / self.track_width) * self.max_velocity) / (curvature.abs() *
        self.max_velocity + (2.0 * self.max_velocity / self.track_width));

        if curvature == 0.0 { 
            max_turn_speed
        } else { 
            let max_slip_speed = (self.friction_coefficient * (1.0 / curvature.abs()) * 9.81 * 39.3701).sqrt(); 
...
impl Trajectory {
    pub fn generate(curve: Bezier, spacing: f64, constraints: TrajectoryConstraints) -> Self {
        Self {
            spacing,
            profile: {
                ...
                while t <= 1.0 {
                    ...
                    forward_pass.push(TrajectoryPoint {
                        ...
                        heading: derivative.y.atan2(derivative.x),
                        distance,
                        curvature,
                    });
